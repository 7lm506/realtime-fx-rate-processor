...
WITH config AS (
    SELECT 
        ...
        3600 AS active_window_seconds,   -- Rates older than this are inactive
        7200 AS scan_window_seconds     -- Scan last 2 minutes for safety
)
...
recent_rates AS (
    ...
    WHERE 
        ...
        event_time >= (
            EXTRACT(EPOCH FROM (
                (SELECT execution_time FROM config) - 
                INTERVAL '60 minutes' --Increased to 60 minutes for testing
            ))::BIGINT * 1000
        )
