...
WITH config AS (
    SELECT 
        ...
        3600 AS active_window_seconds,  -- Rates older than this are considered stale
...
latest_rates AS (
    ...
    WHERE 
        ...
        AND event_time >= (
            EXTRACT(EPOCH FROM (SELECT execution_time FROM config) - INTERVAL '60 minute')::BIGINT * 1000 --Increased to 60 minutes for testing
        )
),
